...
#
# The KERNEL side needs almost nothing — i915 is in-tree and binds at boot; the
# one exception is the initrd, see the bottom of this file. What is
...
{ pkgs, ... }:
{
  ...
  # i915 in the initrd, for the boot splash (../core/plymouth.nix). "Binds at
  # boot" above is about stage 2; the initrd carries nothing it is not told to,
  # so without this Plymouth starts on the EFI framebuffer (simpledrm) and flips
  # resolution the moment i915 binds later — a visible jump on every boot.
